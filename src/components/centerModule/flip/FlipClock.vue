...
export default {
  ...
  methods: {
    getNumber() {
      ...
      setTimeout(() => {
        this.$refs.flipperFirst.flipDown(0, this.$store.state.forthValues.two[0]) //翻牌后数字
      }, 500)
      this.$refs.flipperHour1.setFront(0)
      setTimeout(() => {
        this.$refs.flipperHour1.flipDown(0, this.$store.state.forthValues.two[1])
      }, 600)
      this.$refs.flipperHour2.setFront(0)
      setTimeout(() => {
        this.$refs.flipperHour2.flipDown(0, this.$store.state.forthValues.two[2])
      }, 700)
      this.$refs.flipperMinute1.setFront(0)
      setTimeout(() => {
        this.$refs.flipperMinute1.flipDown(0, this.$store.state.forthValues.two[3])
      }, 700)
      this.$refs.flipperMinute2.setFront(0)
      setTimeout(() => {
        this.$refs.flipperMinute2.flipDown(0, this.$store.state.forthValues.two[4])
      }, 900)
      this.$refs.flipperSecond1.setFront(0)
      setTimeout(() => {
        this.$refs.flipperSecond1.flipDown(0, this.$store.state.forthValues.two[5])
      }, 1100)
      this.$refs.flipperSecond2.setFront(0)
      setTimeout(() => {
        //延时效果
        this.$refs.flipperSecond2.flipDown(0, this.$store.state.forthValues.two[6])
      }, 1500)
...
.xiaoliang .number0:after {
  content: '销';
}
.xiaoliang .number1:before,
.xiaoliang .number1:after {
  content: '量';
}

/*向下翻*/
.xiaoliang .front:before {
  z-index: 3;
}
